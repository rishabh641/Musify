...
import time
import json
from Testing import recommendation_list
import random
...
app = Flask(__name__)
app.secret_key = 'c95a03960e7f42d5bf549d9e7a0fffd5'

My_recommended_songs=[]

Red='http://127.0.0.1:5000/callback'
...
@app.route('/verify')
def verify():
    sp_auth = spotipy.oauth2.SpotifyOAuth(scope=SCOPE,client_id=CLIENT_ID,client_secret=app.secret_key,redirect_uri=Red)
...
def info():
    ...
    return render_template('info.html', songs=songs, artists=artists, similar=recommended_artists, tracks=recommended_songs,recommendation_list=recommendation_list, audio=audio_features, user=user)
...
def get_token(session):
    ...
    if (is_token_expired):
        ...
        sp_oauth = spotipy.oauth2.SpotifyOAuth(scope = SCOPE,client_id=CLIENT_ID,client_secret=app.secret_key,redirect_uri=Red)
...
if __name__ == "__main__":
    # My_recommended_songs=Testing.generate()
    app.run(debug=True)
